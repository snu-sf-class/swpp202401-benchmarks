...
uint64_t Xcalculate(uint64_t *now) {
  ...
  if (Q <= *((uint64_t *)now + 2) && *((uint64_t *)now + 3) <= V)
    return *((uint64_t *)now + 4);
  else {
    if (*((uint64_t **)now) != NULL && Q <= *(*((uint64_t **)now) + 3) &&
        *(*((uint64_t **)now) + 2) <= V)
      value = Xcalculate(*((uint64_t **)now));
    if (*((uint64_t **)now + 1) != NULL &&
        Q <= *(*(((uint64_t **)now) + 1) + 3) &&
        *(*(((uint64_t **)now) + 1) + 2) <= V)
      value = gcd2(value, Xcalculate(*((uint64_t **)now + 1)));
    return value;
  }
}
...
uint64_t Ycalculate(uint64_t l, uint64_t r, uint64_t *now) {
  ...
  if (P <= l && r <= U) {
    if (*((uint64_t **)now + 2) != NULL)
      value = Xcalculate(*((uint64_t **)now + 2));
    return value;
  } else {
    if (P <= m && *((uint64_t **)now) != NULL)
      value = Ycalculate(l, m, *((uint64_t **)now));
    if (m + 1 <= U && *((uint64_t **)now + 1) != NULL)
      value = gcd2(value, Ycalculate(m + 1, r, *((uint64_t **)now + 1)));
    return value;
  }
}
...
void Xupdate(uint64_t *now) {
  ...
  if (*((uint64_t *)now + 2) == Q && Q == *((uint64_t *)now + 3))
    *((uint64_t *)now + 4) = K;
  else {
    if (Q <= m) {
      ...
      if (Q < *(*((uint64_t **)now) + 2) || *(*((uint64_t **)now) + 3) < Q) {
        ...
        *(*((uint64_t **)now) + 3) = m;
        *(*((uint64_t **)now) + 4) = 0;
        if (Q < *((uint64_t *)temp + 2))
          *(*((uint64_t ***)now) + 1) = temp;
        else
          **((uint64_t ***)now) = temp;
        while (1) {
          x = (*(*((uint64_t **)now) + 3) - (*(*((uint64_t **)now) + 2)) + 1) >>
              1;
          y = (*(*((uint64_t **)now) + 2) + (*(*((uint64_t **)now) + 3))) >> 1;
          if (!(Q & x) && !(*((uint64_t *)temp + 2) & x))
            *(*((uint64_t **)now) + 3) = y;
          else if (Q & x && *((uint64_t *)temp + 2) & x)
            *(*((uint64_t **)now) + 2) = y + 1;
          else
            break;
        }
    ...
    if (*((uint64_t **)now + 1) == NULL)
      *((uint64_t *)now + 4) = *(*((uint64_t **)now) + 4);
    else if (*((uint64_t **)now) == NULL)
      *((uint64_t *)now + 4) = *(*(((uint64_t **)now) + 1) + 4);
    else
      *((uint64_t *)now + 4) =
          gcd2(*(*((uint64_t **)now) + 4), *(*(((uint64_t **)now) + 1) + 4));
  }
}
...
void Yupdate(uint64_t l, uint64_t r, uint64_t *now) {
  ...
  if (l < r) {
    ...
    if (*((uint64_t **)now + 1) == NULL)
      K = Xcalculate(*(*((uint64_t ***)now) + 2));
    else if (*((uint64_t **)now) == NULL)
      K = Xcalculate(*(*((uint64_t ***)now + 1) + 2));
    else
      K = gcd2(Xcalculate(*(*((uint64_t ***)now) + 2)),
               Xcalculate(*(*((uint64_t ***)now + 1) + 2)));
  }
  if (*((uint64_t **)now + 2) == NULL) {
